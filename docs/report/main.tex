...
\begin{figure}[H]
	\centering
	\includegraphics[scale=.7]{img/quick-glance-at-data.PNG}
...
\textbf{Một lưu ý}\label{pr:log} $\log$ trong công thức \ref{eq:tfidf} là logarithm tự nhiên\footnote{kí hiệu giáo khoa thường là $\ln$}. Tuy nhiên nhiều tài liệu lại ghi là $\log_2$. Hai khái niệm này bản chất vẫn là nhân cho một hàm $\log$ phù hợp để biểu diễn độ lớn của $df(t)$ so với $N$.
...
\end{proof}
Hiểu nôm na, kết quả $\ln$ chỉ cần nhân hằng số $\log_2(e)$ sẽ thành kết quả $\log_2$. Do đó, sử dụng hàm $\log$ nào không quan trọng.
...
Các mô hình được huấn luyện với bộ dataset như sau:
\begin{itemize}
...
Tiến hành huấn luyện mô hình Decision Tree với các thông số sau:
\begin{itemize}
	\item Bộ phân lớp: \texttt{sklearn.tree.DecisionTreeClassifier}
	\item Tham số \texttt{criterion = 'entropy'} 
\end{itemize}
\begin{figure}[H]
\begin{multicols}{2}
...
Bài toán Hồi quy (Regression) thường áp dụng cho tập nhãn liên tục, tuy nhiên ở đây ta vẫn dùng được cho tập nhãn rời rạc với hai nhãn $\{0, 1\}$. Cụ thể, hàm Logistic là một dạng hàm sigmoid, nhận đầu vào là một số $x$ và cho đầu ra là 0 hoặc 1, đúng như ý đồ mà chúng ta hướng tới: cho ra kết quả 0 (fake) hoặc 1 (true).

Tiến hành huấn luyện mô hình với \texttt{sklearn.linear\_model.LogisticRegression}, dùng các tham số mặc định. 
\begin{figure}[H]
	\begin{multicols}{2}
...
\subsection{Các hướng phát triển xa hơn}
Với những ưu / khuyết điểm đã nêu trên, r
