...
\item Dimensionality Reduction: làm giảm số lượng đặc trưng bằng các thuật toán như PCA, ICA, LDA, NMF hoặc một số kĩ thuật khác; qua đó làm giảm chi phí tính toán và bộ nhớ sử dụng.
...
\begin{proof}
Khi một thuộc tính có dữ liệu hoàn toàn thuộc về một lớp, $\exists i \in [1, n]$ sao cho $p_i = 1$. Hệ quả là tổng $\displaystyle \sum_{i = 1}^{n} (p_i)^2 = 1$ và khi đó theo công thức \ref{eq:gini}, Gini = 0.
\end{proof}
\end{itemize}
